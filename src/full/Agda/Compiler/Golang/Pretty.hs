...
instance Pretty Go.Exp where
  prettyPrec pr e =
    case e of
      ...
      Go.GoLet varName expBody boundedExp -> text varName <+> ":=" <+> pretty expBody <+> "\n" <+> pretty boundedExp
...
instance Pretty Go.GoFunctionSignature where
  ...
  pretty (Go.OuterSignature name genTypes param returnElems returnType) = "func" <+> (pretty name) <+> T.brackets (text ((intercalate ", " genTypes) ++ " any")) <+> (pretty param) <+> (vcat $ map pretty returnElems) <+> (pretty returnType) <+> "{\n"
  ...
  pretty (Go.AnonymousSignature returnType) = "func()" <+> pretty returnType <+> "{\n"